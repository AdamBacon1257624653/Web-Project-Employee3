$(function(){
	...
	$("#QueryBtn").click(function(){
		...
		if(keyword==""){
			$("#keywordError").text("Please type keyword");
		}else{
			ajaxChangePage();
		}
	});
	...
	function updateTable(data){
		...
		var $table=$("<table/>").attr("align","center").attr("cellspacing","0")
			...
			.css("margin-top","30px");
		var $thead=$("<thead/>");
		var theads=["Department","Minimum Salary","Maximum Salary","Employee Amount"];
		for(var i in theads){
		...
		for(var i in jsonObject){
			...
			var $tr=$("<tr/>").attr("id",jobj.salno);
			var $nameTd=$("<td/>").attr("align","center").text(jobj.salname);
			var $losalTd=$("<td/>").attr("align","center").text(jobj.losal);
			var $hisalTd=$("<td/>").attr("align","center").text(jobj.hisal);
			var $empCountTd=$("<td/>").attr("align","center").text(jobj.empCount+"employees");
			$tr.append($nameTd).append($losalTd).append($hisalTd).append($empCountTd);//
			$tbody.append($tr);
		}
